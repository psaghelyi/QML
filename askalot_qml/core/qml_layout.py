...
def _compute_layout_large(graph_data: dict, engine: str = 'dot') -> dict:
    ...
    for edge in edges:
        ...
        if gv_edge:
            ...
            # Non-Graphviz edges — cycle edges get arcs, others get straight lines
            src_node = node_map.get(edge['source'])
            tgt_node = node_map.get(edge['target'])
            if src_node and tgt_node and 'x' in src_node and 'x' in tgt_node:
...
def _compute_layout_full(graph_data: dict, engine: str = 'dot') -> dict:
    ...
    for edge in edges:
        ...
        if gv_edge:
            ...
            # Edges without Graphviz counterpart — cycle edges get arcs,
            # others get straight lines
            src_node = node_map.get(edge['source'])
            tgt_node = node_map.get(edge['target'])
            if src_node and tgt_node and 'x' in src_node and 'x' in tgt_node:
                if edge.get('type') == 'cycle':
